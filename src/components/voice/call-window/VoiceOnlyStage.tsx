...
import { MicOff, VolumeX, UserPlus, MonitorUp, Monitor, MoreHorizontal, X, Hand, Radio } from "lucide-react";
        ...
        >
          {single?.cameraStream ? (
            <>
              ...
              {single.isHandRaised && (
                <div
                  title={`${single.title} levantou a mão`}
                  className="absolute top-3 right-3 p-2 rounded-full bg-amber-400 text-black shadow-lg ring-2 ring-[#0F0F0F] z-20"
                >
            ...
            <>
              ...
              <div className="relative flex items-center justify-center h-44 w-44 sm:h-48 sm:w-48 mb-3">
                ...
                {single?.isHandRaised && (
                  <div
                    title={`${single.title} levantou a mão`}
                    className="absolute -top-1 -right-1 p-2 rounded-full bg-amber-400 text-black shadow-lg ring-2 ring-[#0F0F0F] z-20"
                  >
                    <Hand className="h-3.5 w-3.5" aria-hidden="true" />
                  </div>
                )}
        ...
        <AnimatePresence mode="popLayout">
          {screenFeeds.map((screenFeed) => (
            ...
            >
              ...
              <div
                style={{ cornerShape: "squircle" } as React.CSSProperties}
                className="absolute top-2.5 right-2.5 flex items-center gap-1 px-2 py-1 rounded-[9px] bg-black/50 text-sky-300 text-[10px] font-semibold z-10 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity"
              >
              ...
              >
                ...
                {hasCamera ? (
                  <>
                    ...
                    {feed.isHandRaised && (
                      <div
                        title={`${feed.title} levantou a mão`}
                        className="absolute top-2 right-9 p-1.5 rounded-full bg-amber-400 text-black shadow-md ring-2 ring-[#0F0F0F] z-20"
                      >
                  ...
                  <>
                    ...
                    >
                      ...
                      {feed.isHandRaised && (
                        <div
                          title={`${feed.title} levantou a mão`}
                          className="absolute -top-1 -right-1 p-1.5 rounded-full bg-amber-400 text-black shadow-md ring-2 ring-[#0F0F0F] z-20"
                        >
                          <Hand className="h-3 w-3" aria-hidden="true" />
                        </div>
                      )}
