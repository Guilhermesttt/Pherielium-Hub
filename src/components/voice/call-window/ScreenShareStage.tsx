    ...
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden rounded-[24px] bg-[#0F0F0F]/80 border border-[#161616] shadow-[0_20px_60px_rgba(0,0,0,0.8)]">
      ...
      <div className="absolute top-4 left-4 right-4 flex items-center justify-between pointer-events-none z-20">
        ...
        >
          ...
          {/* Focar / Desfocar Transmissão */}
          {onToggleFocus && (
            <button
              type="button"
              onClick={onToggleFocus}
              className={`flex items-center gap-1.5 h-8 px-2.5 rounded-[10px] text-xs font-semibold transition cursor-pointer ${
                isFocused
                  ? "bg-white text-black shadow-sm"
                  : "text-white/75 hover:text-white hover:bg-white/[0.08]"
              }`}
              title={isFocused ? "Desfocar transmissão (Ver participantes)" : "Focar transmissão (Tela principal)"}
            >
              {isFocused ? (
                <>
                  <Minimize2 className="h-3.5 w-3.5" />
                  <span className="text-[11px] font-bold">Desfocar</span>
                </>
              ) : (
                <>
                  <Maximize2 className="h-3.5 w-3.5" />
                  <span className="text-[11px] font-bold">Focar</span>
                </>
              )}
            </button>
          )}
