...
export const VoiceCallWindow: React.FC<VoiceCallWindowProps> = ({
  ...
  // Discord-style local-only affordances: raise hand and quick reactions are purely
  // visual feedback in this pass (no LiveKit signaling changes), surfaced on the
  // local tile and as a floating burst near the control dock.
    ...
    <AnimatePresence>
      {isOpen && !isDisconnecting && (
        <motion.div
          ...
          className="fixed inset-0 z-9995 flex items-center justify-center p-3 sm:p-5 bg-black/50 backdrop-blur-2xl select-none"
        >
          ...
          >
            ...
            <main className="relative flex-1 w-full min-h-0 overflow-hidden p-3 sm:p-4 flex flex-col items-center justify-center">
              <CallStage
                ...
                onRequestFullscreen={() => setIsStreamFullscreen(true)}
            ...
            {(() => {
              const screenFeeds = activeFeeds.filter((f) => f.isScreen && f.stream);
              const activeScreenFeed = screenFeeds.find((f) => f.id === focusedFeedId) || screenFeeds[0] || null;
              // O filmstrip só é necessário quando o palco principal está dedicado
              // à tela compartilhada em foco — na grade normal, todo mundo (com ou
              // sem câmera) já aparece junto nos cards, sem precisar de uma lista extra.
                  ...
                  {isScreenStageActive && (
                    ...
                    >
                      <ParticipantFilmstrip
                        ...
                        onSelectFeed={handleSelectFeed}
                        onContextMenu={handleFeedContextMenu}
                        onShowAll={() => setIsStreamFocused(false)}
                      />
                    </motion.div>
                  )}
                </AnimatePresence>
            ...
            <footer className="relative w-full pb-4 flex items-center justify-center z-30 shrink-0">
              {/* Floating reaction burst — local visual feedback, rises and fades near the dock */}
              <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 h-0 w-0">
                <AnimatePresence>
                  {activeReactions.map((reaction, index) => (
                    <motion.span
                      key={reaction.id}
                      initial={{ opacity: 0, y: 0, x: (index % 2 === 0 ? -1 : 1) * (8 + index * 6), scale: 0.6 }}
                      animate={{ opacity: 1, y: -90, scale: 1.15 }}
                      exit={{ opacity: 0, y: -130, scale: 0.9 }}
                      transition={{ duration: 1.6, ease: [0.22, 1, 0.36, 1] }}
                      className="absolute bottom-0 left-0 text-3xl select-none"
                    >
                      {reaction.emoji}
                    </motion.span>
                  ))}
              ...
              <CallControlDock
                ...
                onToggleRaiseHand={() => setIsHandRaised((prev) => !prev)}
